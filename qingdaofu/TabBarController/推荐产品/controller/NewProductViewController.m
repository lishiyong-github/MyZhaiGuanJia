...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getRecommendProductslist) name:@"refreshNewProduct" object:nil];
}
...
- (void)goToStoreProductWithModel:(RowsModel *)rowModel andButton:(UIButton *)sender
{
    ...
    [self requestDataPostWithString:saveString params:params successBlock:^(id responseObject) {
        ...
        if ([baseModel.code isEqualToString:@"0000"] || [baseModel.code isEqualToString:@"0003"]) {
            [weakself getRecommendProductslist];
        }
    } andFailBlock:^(NSError *error) {
